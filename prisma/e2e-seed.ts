    ...
    for (const product of group.products) {
      ...
      for (const variant of variants) {
        await prisma.productVariant.create({
          data: { productId, name: variant.name, price: variant.price, sortOrder: variant.sortOrder },
        });
      }
...
async function main() {
  ...
  await seedCatalog();
  console.log("E2E catalog seed completed with deterministic SVG placeholders");
}
